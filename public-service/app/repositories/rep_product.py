...
_IS_ACTIVE = Product.is_active.is_(True)
...
def _build_search_filter(search: str | None) -> list:
    ...
    return [or_(Product.name.ilike(f'%{clean_search}%'), Product.specifications.ilike(f'%{clean_search}%'))]


def _build_filters(search: str | None, category_id: int | None, brand: str | None, requires_installation: bool | None) -> list:
...
def rep_get_products(
    db: Session, search: str | None, limit: int, offset: int, category_id: int | None, brand: str | None, requires_installation: bool | None
):
    filters_ = _build_filters(search, category_id, brand, requires_installation)
    query = (
        select(
            Product.id, Product.name, Product.brand, CategoryProduct.id.label('category_id'), CategoryProduct.name.label('category_name')
        )
...
def rep_count_products(
    db: Session, search: str | None, category_id: int | None, brand: str | None, requires_installation: bool | None
) -> int:
    filters_ = _build_filters(search, category_id, brand, requires_installation)

    query = select(func.count()).select_from(Product).where(_IS_ACTIVE, *filters_)
...
def rep_get_categories_with_products(db: Session, search: str | None):
    ...
    query = (
        ...
        .where(CategoryProduct.is_active.is_(True), _IS_ACTIVE, *filters)
...
def rep_get_brands(db: Session, search: str | None):
    filters = _build_search_filter(search)
    query = select(Product.brand).where(_IS_ACTIVE, *filters).order_by(Product.brand).distinct()
    return db.execute(query).all()
